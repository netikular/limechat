...
class SACrashReporter < NSWindowController
  ...
  def self.developer
    NSBundle.mainBundle.infoDictionary['SACrashReporterDeveloperName']
  end
  ...
  def self.new_crash_log_exists?
    ...
    last_checksum = NSUserDefaults.standardUserDefaults['SACrashReporterLastCheckSum']
  ...
  attr_writer :crashLogDataTextfield
  attr_writer :commentTextfield
  attr_writer :footnoteTextfield
  attr_writer :sendReportButton
  attr_writer :statusSpinner
  attr_writer :statusTextField
  ...
  def sendReport(sender)
    ...
    url = NSURL.URLWithString(NSBundle.mainBundle.infoDictionary['SACrashReporterPostURL'])
    ...
    NSURLConnection.alloc.initWithRequest_delegate(request, self)
  end
