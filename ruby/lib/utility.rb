...
class NSRange
  ...
  def inspect
    "#<#{self.class.to_s} (#{location}, #{length})>"
  end
end
...
class NSWindow
  ...
  def inspect
    "#<#{self.class.to_s}:#{sprintf("0x%x", object_id)} frame=#{frame.to_inspect}, title='#{title}'>"
  end
end
...
class NSFont
  ...
  def inspect
    "#<#{self.class.to_s} name='#{fontName}', pointSize=#{pointSize}#{ bold? ? ', bold' : ''}#{ italic? ? ', italic' : ''}>"
  end
end
...
class NSEvent
  def inspect
    "#<#{self.class.to_s}:#{sprintf("0x%x", object_id)} type=#{_type_name}>"
  end
